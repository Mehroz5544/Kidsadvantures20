...
import { ArrowLeft, RotateCcw, Star, Sparkles, Trophy, Check, X, Calculator } from 'lucide-react';
import { advancedMathQuestions, type AdvancedMathQuestion } from '../data/premiumContent';

type FilterType = 'all' | AdvancedMathQuestion['type'];

const FILTERS: { label: string; value: FilterType }[] = [
  { label: 'All', value: 'all' },
  { label: '×', value: 'multiplication' },
  { label: '÷', value: 'division' },
  { label: '½', value: 'fractions' },
  { label: '.5', value: 'decimals' },
  { label: '%', value: 'percentages' },
  { label: 'x', value: 'algebra' },
  { label: '△', value: 'geometry' },
];

export function AdvancedMath({ onBack }: { onBack: () => void }) {
  ...
  const filteredQuestions = useMemo(() => {
    const list = filter === 'all'
      ...
      : advancedMathQuestions.filter(q => q.type === filter);
    ...
    return (
      <div className="min-h-screen px-4 py-6 flex flex-col items-center justify-center">
        <div className="kids-card bg-white p-8 rounded-3xl shadow-xl w-full max-w-md text-center">
          ...
          <h2 className="text-3xl font-bold text-gray-700 mb-2">Math Quiz Complete!</h2>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="kids-card bg-white p-8 rounded-3xl shadow-xl text-center">
          ...
          <p className="text-xl text-gray-600 mb-4">No questions available for this filter.</p>
    ...
    <div className="min-h-screen px-4 py-6">
      <header className="flex items-center justify-between mb-6">
        ...
        <h1 className="text-2xl font-bold text-white drop-shadow flex items-center gap-2">
          <Calculator className="w-7 h-7" /> Advanced Math
        </h1>
      ...
      <div className="max-w-md mx-auto">
        <div className="kids-card bg-white p-6 rounded-3xl shadow-xl mb-6">
          <div className="flex items-center gap-2 mb-4 justify-center">
            <span className="px-3 py-1 rounded-full text-xs font-bold bg-purple-100 text-purple-600 capitalize">
          ...
          <div className="grid grid-cols-2 gap-3">
            {currentQuestion.options.map((option, index) => (
              <button
                ...
                disabled={answered}
                className={`p-4 rounded-2xl text-xl font-bold transition-all flex items-center justify-between ${
                  !answered
                    ? 'bg-purple-50 hover:bg-purple-100 text-purple-700 hover:scale-105'
              ...
              >
                <span>{option}</span>
                {answered && index === currentQuestion.answer && <Check className="w-5 h-5" />}
                {answered && index === selectedAnswer && index !== currentQuestion.answer && <X className="w-5 h-5" />}
              </button>
            ))}
          </div>

          {answered && currentQuestion.explanation && (
            <div className="mt-4 p-4 rounded-2xl bg-purple-50 animate-slide-up">
        ...
        {answered && (
          <button
            ...
            className="w-full py-4 rounded-2xl bg-gradient-to-r from-purple-400 to-purple-500 text-white font-bold text-lg shadow-xl hover:scale-105 transition-transform"
          >
